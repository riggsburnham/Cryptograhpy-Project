...
namespace CST407_BlowFish
{
    ...
    {
        #region initializing P and S boxes
        string[] P =
        {
        ...
        // use the returned string of this function to display the key as this will have the key used if it was padded
        public string SetupKey(string key)
        {
            var keyBinary = ConvertStringToBinary(key);
            var newKeyBinary = PadKey(keyBinary);
            if (newKeyBinary != keyBinary)
            {
                Console.WriteLine($"Key has been padded, new key = {ConvertBinaryToString(newKeyBinary)}");
            }
            SetupP(newKeyBinary);
            return newKeyBinary;
        }

        private void SetupP(string keyBinary)
        {
            //var hexKey = "aabb09182736ccdd";
            var hexKey = ConvertBinaryStringToHexString(keyBinary);
            int j = 0;
            for (int i = 0; i < P.Length; i++)
            {
                // xor-ing 32-bit parts of the key 
                // with initial subkeys. 
                P[i] = Xor(P[i], hexKey.Substring(j, 8));

                Console.WriteLine("subkey " + (i + 1) + ": " + P[i]);
                j = (j + 8) % hexKey.Length;
            }
        }
        ...
        {
            var plainTextBinary = ConvertStringToBinary(plainText);
            var newPlainTextBinary = PadPlainText(plainTextBinary);
            if (newPlainTextBinary != plainTextBinary)
            {
                Console.WriteLine($"PlainText has been padded, new key = {ConvertBinaryToString(newPlainTextBinary)}");
            }

            var plainTextHex = ConvertBinaryStringToHexString(newPlainTextBinary);
            var encrypted = BlowFishEncrypt(plainTextHex);
            return encrypted;
        }

        private string BlowFishEncrypt(string plainTextHex)
        {
            var newPlainTextHex = "";
            var sb = new StringBuilder();
            for (var j = 0; j < plainTextHex.Length; j += 16)
            {
                newPlainTextHex = plainTextHex.Substring(j, 16);
                for (var i = 0; i < 16; i++)
                {
                    newPlainTextHex = Round(i, newPlainTextHex);
                }
                // postprocessing 
                var right = newPlainTextHex.Substring(0, 8);
                var left = newPlainTextHex.Substring(8, 8);
        ...
        private string Round(int round, string text)
        {
            ...
            Console.WriteLine($"round {round}: {right + left}");

            // swap left and right 
            return right + left;
        }

        private string F(string hex)
        {
            var s = new string[4];
            var col0 = long.Parse(ConvertHexStringToBinaryString(hex.Substring(0, 2))).ToString();
            var col1 = long.Parse(ConvertHexStringToBinaryString(hex.Substring(2, 2))).ToString();
            var col2 = long.Parse(ConvertHexStringToBinaryString(hex.Substring(4, 2))).ToString();
            var col3 = long.Parse(ConvertHexStringToBinaryString(hex.Substring(6, 2))).ToString();
            s[0] = S0[Convert.ToInt32(col0, 2)];
            s[1] = S1[Convert.ToInt32(col1, 2)];
            s[2] = S2[Convert.ToInt32(col2, 2)];
            s[3] = S3[Convert.ToInt32(col3, 2)];
            var result = BinaryAddition(s[0], s[1]);
            result = Xor(result, s[2]);
            result = BinaryAddition(result, s[3]);
            return result;
        }

        public string Decrypt(string cipherTextHex)
        {
            var decryptedHex = BlowFishDecrypt(cipherTextHex);
            var cipherText = ConvertHexStringToString(decryptedHex);
            return cipherText;
        }

        private string BlowFishDecrypt(string cipherTextHex)
        {
            var newCipherTextHex = "";
            var sb = new StringBuilder();
            for (var j = 0; j < cipherTextHex.Length; j += 16)
            {
                newCipherTextHex = cipherTextHex.Substring(j, 16);
                for (var i = 17; i > 1; i--)
                {
                    newCipherTextHex = Round(i, newCipherTextHex);
                }
                // postprocessing 
                var right = newCipherTextHex.Substring(0, 8);
                var left = newCipherTextHex.Substring(8, 8);
                right = Xor(right, P[1]);
                left = Xor(left, P[0]);
                sb.Append(left + right);
            }
        ...
        private string PadKey(string bits)
        {
            if (bits.Length % 32 != 0)
            {
                var sb = new StringBuilder();
                sb.Append(ConvertBinaryToString(bits));
                while (sb.Length % 4 != 0)
                {
                    sb.Append("x");
                }
                //Console.WriteLine($"Key has been padded, new key: {sb}");
                bits = null;
                bits = ConvertStringToBinary(sb.ToString());
            }
            return bits;
        }
        private string PadPlainText(string bits)
        {
            if (bits.Length % 64 != 0)
            {
                var sb = new StringBuilder();
                ...
                //Console.WriteLine($"Plain text has been padded, new plain text: {sb}");
                bits = null;
                bits = ConvertStringToBinary(sb.ToString());
            }
            return bits;
        ...
        {
            a = ConvertHexStringToBinaryString(a);
            b = ConvertHexStringToBinaryString(b);
            var sb = new StringBuilder();
            ...
            return ConvertBinaryStringToHexString(sb.ToString());
        }
        private string BinaryAddition(string hex1, string hex2)
        {
            var bits1 = ConvertHexStringToBinaryString(hex1);
            var bits2 = ConvertHexStringToBinaryString(hex2);
            var sb = new StringBuilder();
            var carryBit = false;
            for (var i = bits1.Length - 1; i >= 0; i--)
            {
                ...
                else if ((bits1[i] == '0' && bits2[i] == '1') || (bits1[i] == '1' && bits2[i] == '0'))
                {
                    sb.Insert(0, carryBit ? "0" : "1");
                }
                else if (bits1[i] == '1' && bits2[i] == '1')
                {
                    sb.Insert(0, carryBit ? "1" : "0");
                    carryBit = true;
                }
            }
            ...
            // perform 2^32 mod, will need to do some trickery since working with binary string...
            if (sb.ToString().Length == 33)
            {
                // in order to mod all you need to do is get rid of the extra bit (2^33 bit) and set the 2^32 bit to 0
                // here we are removing the first 2 bits which are 2^33 and 2^32, then adding back a 0 to the 2^32 position
                sb.Remove(0, 2);
                sb.Insert(0, "0");
            }
        ...
        private string ConvertBinaryToString(string b)
        {
            ...
            {
                byteList.Add(Convert.ToByte(b.Substring(i, 8), 2));
            }
            return Encoding.ASCII.GetString(byteList.ToArray());
        }
        private string ConvertBinaryStringToHexString(string bits)
        {
            return string.Join("",
                Enumerable.Range(0, bits.Length / 8)
                    .Select(i => Convert.ToByte(bits.Substring(i * 8, 8), 2).ToString("X2")));
        }
        private string ConvertHexStringToBinaryString(string hex)
        {
            var hexString = hex.ToString();
            if (hex[1] == 'x')
            {
                hexString = hexString.Remove(0, 2); // gets rid of the 0x leading the hex value
            }
            var sb = new StringBuilder();
            foreach (var h in hexString)
            {
                switch (h)
                {
                    case '0':
        ...
        private string ConvertHexStringToString(string hexString)
        {
            if (hexString == null || (hexString.Length % 2 == 1))
            {
                throw new ArgumentException("Hex string cannot be null or have an odd number of digits");
            }
